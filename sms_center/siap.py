...
def main(argv):
    ...
    s.write('ATE0;+CMGF=0;+CNMI=3,2\r')
    ret=s.readlines()
    '''
...
def readlAllSMS(s):
    ...
    for l in rets:
        if len(l)>20:
            ...
            payload={'smsc':smsc,'tgl':tgl,'sender':nomor,'text':pesan}
	    ret = requests.post("http://35.185.178.17:7744/api/v1/sms", data=json.dumps(payload))
            log(payload)
            total_sms+=1
